...
Future<LoginResponse> signUpApi(Map request) async {
  ...
  return await handleResponse(response).then((json) async {
    ...
    await setValue(
        USER_ROLE_PREF, loginResponse.userData!.user_type.validate());
...
Future<LoginResponse> logInApi(Map request,
  ...
  return await handleResponse(response).then((json) async {
    ...
    if (request['login_type'] == LoginTypeGoogle) {
      ...
      await setValue(
          USER_PHOTO_URL, loginResponse.userData!.profile_image.validate());
    }
...
Future<DishTypModel> getDishTypeList({int? page}) async {
  return DishTypModel.fromJson(await handleResponse(await buildHttpResponse(
      'category-list?page=$page',
      method: HttpMethod.GET)));
}

Future<RecipeStaticModel> getCousinList({String? type}) async {
  return RecipeStaticModel.fromJson(await handleResponse(
      await buildHttpResponse('category-list?type=$type',
          method: HttpMethod.GET)));
}

Future<BaseResponseModel> addIngredients(Map req) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('save-recipe-ingredients',
          request: req, method: HttpMethod.POST)));
}

Future<RecipeStepModel> addRecipeStep(Map req) async {
  return RecipeStepModel.fromJson(await handleResponse(await buildHttpResponse(
      'save-recipe-steps',
      request: req,
      method: HttpMethod.POST)));
}

Future<RecipeIngredientsListModel> getIngredients(
    {int? recipeId, int? page}) async {
  return RecipeIngredientsListModel.fromJson(await handleResponse(
      await buildHttpResponse(
          'recipe-Ingredients-list?recipe_id=$recipeId&page=$page',
          method: HttpMethod.GET)));
}

Future<RecipeStaticModel> addCuisineData(
    {int? page, String? type, String? keyword}) async {
  return RecipeStaticModel.fromJson(await handleResponse(
      await buildHttpResponse(
          'static-data-list?type=$type&keyword=$keyword&page=$page',
          method: HttpMethod.GET)));
}

Future<BaseResponseModel> addUtensilData(Map req) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('save-utensils',
          request: req, method: HttpMethod.POST)));
}

Future<RecipeDashboardModel> getRecipeData() async {
  String userId = appStore.isLoggedIn ? 'user_id=${appStore.userId}' : '';
  logger.v("user log info: $userId");
  final response = RecipeDashboardModel.fromJson(await handleResponse(
      await buildHttpResponse('dashboard-detail?$userId&status=1',
          method: HttpMethod.GET)));
  logger.v("response in getRecipeData => $response");
  // return RecipeDashboardModel.fromJson(await handleResponse(
  //     await buildHttpResponse('dashboard-detail?$userId&status=1',
  //         method: HttpMethod.GET)));
...
Future<RecipeDetailModel> getRecipeDetailData({int? recipeID}) async {
  String userId = appStore.isLoggedIn ? '&user_id=${appStore.userId}' : '';
  return RecipeDetailModel.fromJson(await handleResponse(
      await buildHttpResponse('recipe-detail?recipe_id=$recipeID$userId',
          method: HttpMethod.GET)));
}

Future<RecipeListModel> getRecipeListData({int? status, int? page}) async {
  String userId = appStore.isLoggedIn ? '&user_id=${appStore.userId}' : '';
  return RecipeListModel.fromJson(await handleResponse(await buildHttpResponse(
      'recipe-list?$userId&status=$status&page=$page',
      method: HttpMethod.GET)));
}

Future<RecipeRatingModel> addRating(Map req) async {
  return RecipeRatingModel.fromJson(await handleResponse(
      await buildHttpResponse('save-rating',
          request: req, method: HttpMethod.POST)));
}

Future<RecipeRatingModel> deleteRating(Map req) async {
  return RecipeRatingModel.fromJson(await handleResponse(
      await buildHttpResponse('delete-rating',
          request: req, method: HttpMethod.POST)));
}

Future<LikeResponseModel> addLike(Map req) async {
  return LikeResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('save-like',
          request: req, method: HttpMethod.POST)));
}

Future<LikeResponseModel> removeLike(Map req) async {
  return LikeResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('delete-like',
          request: req, method: HttpMethod.POST)));
}

Future<RecipeBookMarkResponse> addBookMark(Map req) async {
  return RecipeBookMarkResponse.fromJson(await handleResponse(
      await buildHttpResponse('save-bookmark',
          request: req, method: HttpMethod.POST)));
}

Future<RecipeBookMarkResponse> removeBookMark(Map req) async {
  return RecipeBookMarkResponse.fromJson(await handleResponse(
      await buildHttpResponse('delete-bookmark',
          request: req, method: HttpMethod.POST)));
}

Future<RecipeModel> deleteRecipe(req) async {
  return RecipeModel.fromJson(await handleResponse(await buildHttpResponse(
      'delete-recipe',
      request: req,
      method: HttpMethod.POST)));
}
...
Future<RecipeBookMarkModel> getBookMarkData({int? perPage, int? page}) async {
  return RecipeBookMarkModel.fromJson(await handleResponse(
      await buildHttpResponse(
          'get-user-bookmark?user_id=${appStore.userId}&page=$page',
          method: HttpMethod.GET)));
}

Future<RecipeStepModelList> getRecipeStepModel(
    {int? page, int? recipeID}) async {
  return RecipeStepModelList.fromJson(await handleResponse(
      await buildHttpResponse(
          'recipe-steps-list?recipe_id=$recipeID&page=$page',
          method: HttpMethod.GET)));
}

Future<RecipeIngredientsModel> deleteIngredients(req) async {
  return RecipeIngredientsModel.fromJson(await handleResponse(
      await buildHttpResponse('delete-recipe-ingredients',
          request: req, method: HttpMethod.POST)));
}
...
Future<RecipeListModel> recipeSearchData({int? page, String? keyword}) async {
  return RecipeListModel.fromJson(await handleResponse(await buildHttpResponse(
      'recipe-list?page=$page&keyword=$keyword&status=1',
      method: HttpMethod.GET)));
}

Future<void> addDishTypeList(Map req) async {
  await handleResponse(await buildHttpResponse('save-category',
      request: req, method: HttpMethod.POST));
}

Future<void> deleteDishTypeList(Map req) async {
  return await handleResponse(await buildHttpResponse('delete-category',
      request: req, method: HttpMethod.POST));
}

Future<void> addCuisine(Map req) async {
  return await handleResponse(await buildHttpResponse('save-static-data',
      request: req, method: HttpMethod.POST));
}

Future<void> deleteCuisine(Map req) async {
  return await handleResponse(await buildHttpResponse('delete-static-data',
      request: req, method: HttpMethod.POST));
}
...
Future<RecipeSliderModel> getSliderData({int? page}) async {
  return RecipeSliderModel.fromJson(await handleResponse(
      await buildHttpResponse('slider-list?page=$page',
          method: HttpMethod.GET)));
}

Future<RecipeSliderModel> removeSliderData(Map req) async {
  return RecipeSliderModel.fromJson(await handleResponse(
      await buildHttpResponse('delete-slider',
          request: req, method: HttpMethod.POST)));
}
...
Future<int> addStepData(
    {String? description,
    String? ingredientUseId,
    File? file,
    int? recipeId,
    int? id,
    Uint8List? recipeStepWeb}) async {
  int? stepId;
  MultipartRequest multiPartRequest =
      await getMultiPartRequest('save-recipe-steps');
  ...
  if (isWeb) {
    if (recipeStepWeb != null) {
      multiPartRequest.files.add(MultipartFile.fromBytes(
          'recipe_step_image', recipeStepWeb,
          filename: '.jpg'));
    }
  } else {
    if (file != null) {
      multiPartRequest.files
          .add(await MultipartFile.fromPath('recipe_step_image', file.path));
    }
  }
...
Future<int> addUpdateRecipeData({int? id, File? file, int? status}) async {
  ...
  if (newRecipeModel != null) {
    ...
    multiPartRequest.fields['portion_unit'] =
        newRecipeModel!.portionUnit.validate().toString();
    multiPartRequest.fields['portion_type'] =
        newRecipeModel!.portionType.validate().toString();
    multiPartRequest.fields['difficulty'] =
        newRecipeModel!.difficulty.validate();
    multiPartRequest.fields['preparation_time'] =
        newRecipeModel!.preparationTime.validate().toString();
    multiPartRequest.fields['baking_time'] =
        newRecipeModel!.bakingTime.validate().toString();
    multiPartRequest.fields['resting_time'] =
        newRecipeModel!.restingTime.validate().toString();
    multiPartRequest.fields['dish_type'] =
        newRecipeModel!.dish_type.validate().toString();
    multiPartRequest.fields['cuisine'] =
        newRecipeModel!.cuisine.validate().toString();
  ...
  if (isWeb) {
    if (newRecipeModel!.fileBytes != null) {
      multiPartRequest.files.add(MultipartFile.fromBytes(
          'recipe_image', newRecipeModel!.fileBytes!,
          filename: '.jpg'));
    }
  } else {
    if (file != null)
      multiPartRequest.files
          .add(await MultipartFile.fromPath('recipe_image', file.path));
  }
...
Future updateProfile(
  ...
  if (isWeb) {
    multiPartRequest.files.add(
        MultipartFile.fromBytes('profile_image', fileWeb!, filename: '.jpg'));
  } else {
    if (file != null)
      multiPartRequest.files
...
Future<void> addSliderData(
  ...
  if (isWeb) {
    if (imageWeb != null) {
      multiPartRequest.files.add(
          MultipartFile.fromBytes('slider_image', imageWeb, filename: '.jpg'));
    }
  } else {
    if (file != null) {
      multiPartRequest.files
          .add(await MultipartFile.fromPath('slider_image', file.path));
    }
  }
